...
Uso:
    ...
    4) Quando solicitado, cole a EXTERNAL Database URL do PostgreSQL do Render.
       A URL não é gravada em arquivo e não é exibida na tela.
"""
...
def main() -> int:
    print("=" * 68)
    print(" CATÁLOGO DIGITAL — MIGRAÇÃO SQLITE -> POSTGRESQL")
    print("=" * 68)
    print()
    print("Este processo COPIA os dados. O catalogo.db local não será apagado.")
    print("O PostgreSQL de destino precisa estar vazio (somente migrations).")
    print()
    ...
    backup_path = source_path.with_name("catalogo-backup-antes-producao.db")
    if not backup_path.exists():
        print("ERRO: o backup catalogo-backup-antes-producao.db não foi encontrado.")
    ...
    print()
    print("IMPORTANTE: use a EXTERNAL URL aqui, não a Internal URL.")
    ...
    try:
        ...
        with source_engine.connect() as source_conn, target_engine.connect() as target_conn:
            ...
            if source_version != target_version:
                ...
                print("Não faremos a cópia para evitar corrupção de dados.")
                return 1
            ...
            source_only = sorted(
                name for name in source_meta.tables
                if name != ALEMBIC_TABLE and name not in target_meta.tables
            )
            if source_only:
                print("\nAVISO: tabelas locais sem equivalente online serão ignoradas:")
                for name in source_only:
                    print(f"  - {name}")

            # Proteção: o banco de destino deve estar vazio.
            non_empty_target = []
            ...
            if non_empty_target:
                ...
                print("O PostgreSQL online já possui dados:")
                ...
                print("Não execute seed no banco online antes da migração.")
                return 2
            ...
            print()
            print("Nenhum dado foi alterado ainda.")
            confirmation = input(
        ...
        # Uma transação única: se qualquer tabela falhar, tudo é revertido.
        with source_engine.connect() as source_conn, target_engine.begin() as target_conn:
            ...
            for target_table in target_tables:
                ...
                print(f"  OK  {target_table.name:<32} {qty:>6} registro(s)")

            # Como os IDs foram copiados explicitamente, sincroniza sequences do PostgreSQL.
            for table in target_tables:
        ...
        if differences:
            ...
            print("Revise antes de usar o ambiente online.")
            return 3
        ...
        print("O banco SQLite e seu backup continuam intactos no computador.")
        print()
        print("Próximo teste:")
        print("  1) Abra o admin online.")
        print("  2) Entre com seu usuário que funcionava localmente.")
        print("  3) Confira lojas, serviços, produtos e agendamentos.")
        print()
        print("Observação: arquivos de imagem da pasta local uploads não são")
        print("copiados por este script. Vamos tratar o armazenamento de imagens")
        print("de produção em uma etapa separada.")
        return 0
        ...
        print(safe_message)
        print()
        print("Nenhum dado do SQLite foi apagado.")
        print("Se a falha ocorreu durante a cópia, a transação do PostgreSQL")
